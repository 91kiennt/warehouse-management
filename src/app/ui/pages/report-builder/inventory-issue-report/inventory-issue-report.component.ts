...
export interface ReportRow {
    ...
    index?: number;
    id?: number;        // ID của phiếu xuất kho
...
@Component({
    standalone: true,
    selector: "app-inventory-issue-report",
...
export class InventoryIssueReportComponent implements OnInit, OnDestroy {
    ...
    async onGetData(): Promise<void> {
        try {
            ...
            const filtered = issues.filter(i => {
                const pDate = i.postingDate || (i as any).posting_date;
                return pDate >= this.startDate && pDate <= this.endDate;
            });

            // Sort by posting date then issue number
            filtered.sort((a, b) => {
                const dateA = a.postingDate || (a as any).posting_date;
                const dateB = b.postingDate || (b as any).posting_date;
                if (dateA !== dateB) return dateA.localeCompare(dateB);
                
                const numA = a.issueNumber || (a as any).issue_number;
                const numB = b.issueNumber || (b as any).issue_number;
            ...
            for (const iss of filtered) {
                const issueNum = iss.issueNumber || (iss as any).issue_number || "";
                const postDate = iss.postingDate || (iss as any).posting_date || "";
                ...
                try {
                    ...
                    console.error("Lỗi parse items cho phiếu:", issueNum, e);
                }

                if (itemsList.length === 0) continue;

                // Calculate totals for this issue
                let docQty = 0;
                let docAmount = 0;
            ...
            if (filtered.length === 0) {
                this.showFeedback("Không tìm thấy dữ liệu phiếu xuất trong khoảng thời gian đã chọn.", "error");
            } else {
                this.showFeedback(`Đã tải thành công ${filtered.length} phiếu xuất.`);
            }
    ...
    onEditIssue(id: number | undefined): void {
        if (id) {
            this.router.navigate(["/warehouse-issues"], { queryParams: { id } });
        }
    }
